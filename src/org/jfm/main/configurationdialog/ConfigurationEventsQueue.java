...
import org.jfm.event.FSPluginOptionsChangedEvent;
...
public class ConfigurationEventsQueue {
	private static Set<BroadcastEvent> events=new HashSet<BroadcastEvent>();
	...
	public static void notifyPendingEvents(){
		Iterator<BroadcastEvent> it=events.iterator();
		while(it.hasNext()){
			BroadcastEvent ev=it.next();
			switch(ev.getType()){
				...
				case BroadcastEvent.CHANGE_HELP_URL:
					...
					break;
				case BroadcastEvent.FSPLUGIN_EVENT:
					Broadcaster.notifyFSPluginOptionChangedListeners((FSPluginOptionsChangedEvent)ev);
					break;
			}
		}		
		events.clear();
